...
<body>    <h1>Extended WEATHER</h1></body></html>''')
#Get variables from address field
...
#marine 5 day location forecast
print ('<h2></h2>')
url = 'https://marine.weather.gov/MapClick.php?lat='+str(lat)+'&lon='+str(lng)+'&unit=0&lg=english&FcstType=dwml'
print ('<h1>Coordinate Location JAYTVISION:</h1>')
print ('<h4></h4>')
source = urllib.request.urlopen(url).read()
forecast = bs.BeautifulSoup(source, 'lxml')
...
with requests.Session() as s:
    ...
    for row in my_list4[1:8]:
        ...
        sst.append(row)
#############################################################################
print ('''<section><div id="container2">
<div><h2>    SST  :  </h2></div>
<marquee>'''+str(sst)+'''</marquee>
<h4></h4>
<div><h2>    5 day Marine Forecast usually with waves by the sea:</h2></div>
<h4></h4>
<div>'''+str(html)+'''</div>
<h4></h4>
<div><h2>    5 day JayT GFS Forecast:</h2></div>
<marquee>'''+str(forecast)+'''</marquee>
<h4></h4>
...
<marquee>'''+str(waves)+'''</marquee>
<marquee>'''+str(waves2)+'''</marquee>
<marquee>'''+str(wavesloc)+'''</marquee>
<h4></h4>
</div></section>''')
